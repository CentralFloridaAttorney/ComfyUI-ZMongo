...
def _chunk_text_for_storage(text: str, *, chunk_size: int, chunk_overlap: int) -> list[dict[str, Any]]:
    if not text:
        return []

    safe_size = max(250, int(chunk_size or 3000))
    safe_overlap = max(0, min(int(chunk_overlap or 0), safe_size - 1))
    chunks: list[dict[str, Any]] = []
    start = 0
    text_length = len(text)

    while start < text_length:
        end = min(text_length, start + safe_size)
...
def _chunks_by_index(chunks: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {f"c{int(chunk.get('index') or 0):04d}": chunk for chunk in chunks}
...
def _best_text_from_document(document: Any) -> tuple[str, str, int]:
    ...
    # Also consider chunk text when a raw text field is missing.
    marker = object()
    chunk_zero = _safe_get_by_path(document, "document_text.chunks_by_index.c0000.text", marker)
    if chunk_zero is not marker:
        candidates.append(("document_text.chunks_by_index.c0000.text", chunk_zero))
...
class ZMongoDocumentIngestTextFile(AlwaysDirtyMixin):
    ...
    chunk it, and save the result as a ZMongo text-document record.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "chunk_text": ("BOOLEAN", {"default": True}),
                "chunk_size": ("INT", {"default": 3000, "min": 250, "max": 100000}),
                "chunk_overlap": ("INT", {"default": 300, "min": 0, "max": 50000}),
    ...
    def ingest_text_file(
        ...
        chunks = _chunk_text_for_storage(
            extracted_text,
            chunk_size=int(chunk_size),
            chunk_overlap=int(chunk_overlap),
        ) if bool(chunk_text) else []
        ...
        text_payload = {
            ...
            "chunked": bool(chunk_text),
            "chunk_size": int(chunk_size),
            "chunk_overlap": int(chunk_overlap),
            "chunk_count": len(chunks),
            "chunks": chunks,
            "chunks_by_index": _chunks_by_index(chunks),
            "extraction_engine": extraction_engine,
        }
