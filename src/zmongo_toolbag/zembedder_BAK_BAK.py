...
from bson.objectid import ObjectId # Added for demo
...
# The gemini_embedding_model module would also need the 'import os' fix from a previous step
# if it hasn't been added yet, but we focus on ZEmbedder here.
...
class ZEmbedder:
    ...
    def _get_nested_field(doc: Dict[str, Any], field_path: str) -> Any:
        """
        Safely get nested values from a document using a dotted path, e.g.
        'embedding.case_name' -> doc['embedding']['case_name'].
        """
        if not isinstance(doc, dict) or not field_path:
            return None
        ...
        **kwargs: Any,
        ...
        if not text and collection and document_id is not None:
            fetch_result = await self._await_repo_result(
                self.repository.find_one_async(collection, {"_id": document_id})
            )
        ...
        if collection and document_id is not None:
            # Explicitly create the full $set update document once.
            update_payload = {"$set": {embedding_field: vectors}}

            try:
                if hasattr(self.repository, "update_document"):
                    repo_call = self.repository.update_document(
                        collection,
                        {"_id": document_id},
                        update_payload,  # Use the explicit $set payload
                    )
    ...
    def get_embedding_sync(self, *args: Any, **kwargs: Any) -> SafeResult:
        ...
        """
        # FIX: Simplify the synchronous execution entry point.
        try:
            # Explicitly run the async coroutine to completion
            result = asyncio.run(self.get_embedding(*args, **kwargs))
        except Exception as exc:
            # Handle any exception during the synchronous execution of the async task
            return SafeResult.fail(f"get_embedding_sync execution error: {exc}")

        # The result returned by asyncio.run should be the SafeResult from get_embedding
        return result if isinstance(result, SafeResult) else SafeResult.ok(result)

    # ------------------------------------------------------------------
    # Utility: cosine similarity (UNCHANGED)
    # ------------------------------------------------------------------
    @staticmethod
    ...
    # ------------------------------------------------------------------
    # Utility: batch embedding (already-chunked texts) (UNCHANGED)
    # ------------------------------------------------------------------
...
# ----------------------------------------------------------------------
def _single_record_demo(collection: Optional[str]=None, doc_id_str: Optional[str]=None, text_field: Optional[str]=None, embedding_field: Optional[str]=None) -> None:
    """
    Demo: embed a single field on a single record using Gemini + chunking.
    """
    import sys
    # Note: zmongo.py must be importable
    from .zmongo import ZMongo

    logging.basicConfig(level=logging.INFO)

    collection = collection or "case_briefs"
    doc_id_str = doc_id_str or "683d1afea00293c904762fd5"
    text_field = text_field or "irac.raw_text"
    embedding_field = embedding_field or "embedding.irac.raw_text"
    ...
    try:
        # FIX APPLIED HERE: Set skip_if_present to True to prevent wasting API calls.
        res = embedder.get_embedding_sync(
            ...
            chunk_size=1500,
            overlap=150,
            skip_if_present=True, # <--- CHANGED FROM FALSE TO TRUE
            as_safe_result=True,
        )
        ...
        data = res.data or {}
        # If skip works, this status message will indicate it was skipped.
        if data.get("from_cache"):
            logger.info("✅ Document %s skipped (embedding already present).", doc_id_str)
            return
...
def all_record_demo(
    collection: str = "case_briefs",
    text_field: str = "irac.raw_text",
    embedding_field: str = "embedding.irac.raw_text",
) -> None:
    """
    Batch demo: iterate over ALL documents in `collection`, generating
    embeddings for `text_field` -> `embedding_field`.

    This uses the SAME doc_id-based get_embedding() path as the single-record
    demo, and then explicitly writes the vectors back to Mongo to guarantee
    persistence.
    """
    import sys
    from .zmongo import ZMongo
    ...
    # 768 is your current Gemini dim
    output_dim = 768

    repo = ZMongo()
    embedder = ZEmbedder(repository=repo, output_dimensionality=output_dim)

    # Rate limiting: 10 docs, then 2s pause (what your logs show)
    BATCH_SIZE = 10
    ASYNC_DELAY_SECONDS = 2.0

    async def _process_all():
        logger.info("🚀 Starting batch embedding for collection: '%s'", collection)

        # Fetch docs that HAVE text_field; we let skip_if_present & explicit
        # save logic decide what to do per-record.
        ...
        if not res.success:
            logger.error("❌ Failed to query documents: %s", res.error)
            return

        docs = res.data or []
        total = len(docs)
        logger.info("Found %d documents to process.", total)
        ...
        for i, doc in enumerate(docs):
            ...
            # === CORE: SAME PATH AS SINGLE-RECORD DEMO ===
            result = await embedder.get_embedding(
                ...
                chunk_size=1500,
                overlap=150,
                skip_if_present=True,  # only embed when missing
                as_safe_result=True,
            )

            if not result.success:
                stats["failed"] += 1
                logger.warning("⚠️ Failed doc %s: %s", doc_id, result.error)
                continue
            ...
            # If we hit skip_if_present cache successfully
            if from_cache and vectors:
                stats["skipped"] += 1
            else:
                # We *just* generated embeddings. To absolutely guarantee
                # persistence, explicitly write them back here.
                if not vectors:
                    stats["failed"] += 1
                    logger.warning("⚠️ No vectors returned for doc %s", doc_id)
                    continue

                save_res = await embedder._await_repo_result(
                    repo.update_one(
                        collection,
                        {"_id": ObjectId(doc_id)},
                        {"$set": {embedding_field: vectors}},
                    )
                )
                if not save_res.success:
                    stats["failed"] += 1
                    logger.warning(
                        "⚠️ Failed to save embeddings for %s: %s",
                        doc_id,
                        save_res.error,
                    )
                    continue
            ...
            # --- Batching & pause logic ---
            current_count = i + 1

            if current_count % BATCH_SIZE == 0 or current_count == total:
                logger.info(
                    "Progress: %d/%d | ✅ New: %d | ⏭️ Skipped: %d | ❌ Failed: %d",
                    current_count,
                    total,
                    stats["success"],
                    stats["skipped"],
            ...
            if (
                ...
                and current_count < total
            ):
                logger.info(
                    "⏳ Pausing for %.1f seconds to respect API rate limits...",
                    ASYNC_DELAY_SECONDS,
                )
                await asyncio.sleep(ASYNC_DELAY_SECONDS)
                # If you *want* a tiny per-record pause, uncomment:
...
if __name__ == "__main__":
    ...
    # Simple mode switch:
    #   python zembedder.py --all        -> batch mode
    #   python zembedder.py              -> single-record demo
    if "--all" in sys.argv:
        all_record_demo()
    else:
        _single_record_demo()
