...
class MongoOneHotDB:
    ...
    def __init__(self, zmongo: Optional[ZMongo]=None, collection: str='onehot_words', *, create_indexes: bool=True):
    ...
    async def _get_next_index_async(self) -> int:
        ...
        res = await self._zmongo.find_many_async(coll=self._collection, query={}, sort=[('index', -1)], limit=1)
    ...
    async def _add_word_async(self, word: str) -> SafeResult:
        ...
        if existing.success and existing.data:
            updated = await self._zmongo.update_one_async(self._collection, {'word': w}, {'$set': {'updated_at': datetime.now()}})
            if not updated.success:
                return updated
            return existing
        next_idx = await self._get_next_index_async()
        doc = {'word': w, 'index': next_idx, 'created_at': datetime.now(), 'updated_at': datetime.now()}
