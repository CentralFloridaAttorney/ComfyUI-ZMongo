...
def _load_route_modules(route_modules: Iterable[Tuple[str, str]]) -> None:
    ...
    for module_basename, log_name in route_modules:
        try:
            import_module(f".{module_basename}", package=package_name)
            logger.debug("Loaded route module: %s.%s", package_name, module_basename)
        except Exception:
            logger.exception("Failed to load %s", log_name)


def _load_node_modules(modules_to_load: Iterable[Tuple[str, str]]) -> None:
    package_name = _get_package_name()
...
MODULES_TO_LOAD = [
    ...
    ("zmongo_remote_nodes", "nodes.zmongo_remote_nodes"),
]

_load_route_modules(ROUTE_MODULES_TO_LOAD)
_load_node_modules(MODULES_TO_LOAD)
